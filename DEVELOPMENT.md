...
**Patch.** Implements the seven decided conflicts captured in `docs/domain_conflicts.json` after operator review of the V2.14.15 conflict review board. Five concerns:

1. **Stock Saw on stands 7-8 for ALL events (pro AND college).** Operator override: drops the older pro/college stand distinction. `STAND_CONFIGS['stock_saw']` now `specific_stands=[7, 8]` with labels `Stand 7`, `Stand 8` (was `[1, 2]`). `services.heat_generator._stand_numbers_for_event` and `_is_stock_saw` no longer gate on `event.event_type == 'college'` — the [7, 8] override applies to any event named "Stock Saw". `services.flight_builder._CONFLICTING_STANDS` now lists `stock_saw ↔ saw_hand` because group [5-8] of the saw_hand stands physically overlaps the stock-saw stands. Old `stock_saw ↔ hot_saw` adjacency kept defensively. `docs/DOMAIN_CONTRACT.md` and `FlightLogic.md` updated; `services/scratch_cascade.py` comment refreshed. Test `test_pro_stock_saw_not_overridden` renamed to `test_pro_stock_saw_uses_stands_7_and_8` with inverted assertion; new `test_pro_stock_saw_solo_alternates_7_8` regression test.

2. **Robust partner pairing rewrite.** `services/partner_matching.py::auto_assign_event_partners` rewritten to use the fuzzy `services.name_match.find_partner_match` ladder (catches Mckinley/Mickinley typos, first-name-only entries, etc.) instead of plain normalized lookups, and to track CLAIMS before throwing anyone into the unclaimed pool. Three-phase resolver: (1) walk pool, fuzzy-resolve each comp's partner field, write canonical names back on confirmed reciprocal pairs (heals typos); (2) one-sided claims (A→B with B→blank or B→C) hold BOTH for review and are NOT thrown into the auto-pair pool — the fix the operator explicitly called out; (3) genuinely unclaimed pool gets auto-paired with mixed-gender priority for mixed events. Returns 3-bucket summary (`confirmed_pairs` / `assigned_pairs` / `one_sided_claims`) with reason codes (`unresolved`, `self_reference`, `non_reciprocal`, `partner_already_paired`, `one_sided_claim`). `routes/registration.py::auto_assign_pro_partners_route` flash now surfaces all three buckets so operators see actionable next steps instead of an opaque "unmatched" count.

3. **Heat.competitors ↔ HeatAssignment sync invariants pinned.** Audited all 10 mutation sites in routes + services — every one already calls `heat.sync_assignments(comp_type)`. No production fix needed. New `tests/test_heat_sync_invariants.py` (5 tests) asserts `JSON_ids == HeatAssignment_ids` after `set_competitors`, `add_competitor`, `remove_competitor`, heat generation, and full scratch cascade. Catches future drift before it ships rather than relying on each callsite to remember.

4. **Preflight hard-blocker codes defined.** `services/preflight.BLOCKING_CODES` constant + `get_blocking_issues()` helper formalize which preflight issues hard-block heat generation (per DOMAIN_CONTRACT): `unresolved_partner_name`, `self_reference_partner`, `non_reciprocal_partnership`, `heat_sync_mismatch`. `build_preflight_report()` now returns `report['blocking']` + `report['has_blockers']`. `templates/scheduling/preflight.html` shows a red blocker banner when `has_blockers` is True. `routes/scheduling/preflight.py::preflight_json` exposes per-issue `blocking: true/false` + `blocking_count` for the events-page banner. New `TestBlockingCodes` class in `tests/test_preflight.py` (3 tests).

5. **Domain Conflict Review Board (continuation from V2.14.15).** All 6 of 7 actionable conflicts in `docs/domain_conflicts.json` flipped to `implemented`. The 7th (`local-sqlite-vs-production-postgres`) stays `accepted_contract` — ongoing CI discipline (postgres-smoke job already in workflow), no single fix.

**Files modified:** `config.py`, `services/heat_generator.py`, `services/flight_builder.py`, `services/scratch_cascade.py`, `services/partner_matching.py`, `services/preflight.py`, `routes/scheduling/preflight.py`, `routes/registration.py`, `templates/scheduling/preflight.html`, `docs/DOMAIN_CONTRACT.md`, `FlightLogic.md`, `docs/domain_conflicts.json`, `pyproject.toml`, `routes/main.py` (both `/health` literals), `CLAUDE.md`, `tests/test_stock_saw_stand_rebalance.py`, `tests/test_heat_generator.py`, `tests/test_preflight.py`. **New test files:** `tests/test_partner_auto_assign_v2.py` (6 tests), `tests/test_heat_sync_invariants.py` (5 tests). 262 tests pass on the focused suite. No migration.

### 2026-04-23 (V2.14.14)

**Patch.** College Birling bracket generation rewritten to use the actual competitor count instead of rounding up to the next power of two. Before: 12 competitors produced a 16-slot bracket with 4 first-round BYE slots attached to seeds 1-4; the extra round and bye-matches inflated the losers-bracket shape and forced re-derivation of match counts via a formula that drifted from reality on non-power-of-two fields, producing visually-misaligned rounds and unreachable matches. After: 12 competitors → 6 first-round matches (1v12, 2v11, 3v10, 4v9, 5v8, 6v7) with zero byes; odd fields give the top seed a single first-round bye and mirror-pair the rest. Winners rounds now compute via `ceil(n/2)`, losers rounds consume the actual winners-round-count list, and `_sweep_winners_byes` cascades through every round (not just round 1) so a match with one live competitor and a dead source match auto-advances. Algorithm change in `services/birling_bracket.py` (+117/-45); 12-competitor regression tests in `tests/test_birling_bracket_12.py` updated to assert the compact shape. **CI health carry-ons:** ruff I001 fix on `tests/test_partner_pairing_fixes.py`, adjacency threshold loosened to 15% on `tests/test_flight_builder_25_pros.py::test_no_competitor_in_consecutive_heats` (eliminates stochastic flake from tied-sort-key DB ordering), and `tests/test_schedule_generation.py` mock signature updated to match the V2.14.2 `build_pro_flights(tournament, num_flights=None, commit=True)` signature. No migration.
...
**What changed for you.** Two race-day-blocking issue clusters fixed in one release.

**Cluster 1 — persisted-config completeness.** The "Generate All Heats & Build Flights" button on the Pro Flights page silently ignored the operator's saved flight count, falling back to the builder's 8-heats-per-flight default. With ~615 pro heats this produced 77 flights even when the operator had explicitly set 4. Audit of the broader pattern surfaced two more session-only reads that lost operator-persisted choices on a fresh browser session: Preflight autofix dropped Saturday spillover IDs (Chokerman Run 2 orphaned), and the Day Schedule print silently omitted FNF + spillover events. Plus a latent bug: the async generate-all path skipped relay + spillover integration entirely.

**Cluster 2 — partner pairing.** Five distinct bugs in the Jack & Jill / Double Buck / Pulp Toss / Peavey / Pro Partnered Axe pairing pipeline, all surfaced by an in-the-wild "Jordan Navas (UM-A) & McKinley" heat where McKinley had no school tag. Heat-gen's `_find_partner` was strict (exact + first-name only) while excel_io's importer had Levenshtein fuzz with a comment literally saying "handles typos like McKinley/Mickinley" — so manually-entered typos survived to heat-gen and silently placed competitors solo on a stand. The render fallback then displayed the bare partner string from the form with no school tag because the partner had landed in a different heat. Preflight's odd-pool check was pro-only (college-side partnered events had no check), and there was no preflight surface for unresolved-partner names or non-reciprocal partnerships.

**Why it matters.** Race-day partnered-event entrants with bad partner data used to silently land on heat sheets one-half-of-a-pair, with no school tag, no preflight warning, and no way to find them before they walked up to a stand without a partner. With race day in days, this fix bundle (a) makes every persisted-config trigger path consistent (one resolver helper used by all callers), (b) extends the partner-name matcher to Levenshtein ≤ 2 across heat-gen + render + preflight via a shared `services/name_match.py` module, (c) holds unpaired entrants OUT of heats by default with a flash linking to Preflight, (d) renders partner names with school tags via a tournament-wide roster fallback, and (e) gives the operator a Preflight Check page that lists every unresolved + non-reciprocal pair with the unmatched partner string visible.

**Files (Cluster 1).** `routes/scheduling/flights.py` (+30, `_resolve_num_flights_from_persisted_config` helper + threaded into `one_click_generate`), `routes/scheduling/preflight.py` (+5, DB-first read for autofix POST), `routes/scheduling/heat_sheets.py` (+5, DB-first read for day-schedule print), `services/schedule_generation.py` (+50, async generate now chains relay + spillover atomically with commit=False), `tests/test_one_click_and_fnf.py` (+90, 3 new regression tests for persisted-num_flights honour), `tests/test_persisted_config_route_reads.py` (new, 5 tests covering session-only fixes + async chain).

**Files (Cluster 2).** `services/name_match.py` (new, 130 lines — shared 3-tier matcher: exact → first-name → Levenshtein ≤ 2, refuses on ambiguity at tiers 2-3), `services/heat_generator.py` (+90, `_find_partner` delegates to `name_match`, `_build_partner_units` defaults to `skip_unpaired=True` and populates `unpaired_log`, new `get_last_unpaired_partnered()` accessor, missing-competitor warning excludes held-back IDs), `services/partner_resolver.py` (+45, `lookup_partner_cid` delegates to `name_match`, `pair_competitors_for_heat` accepts optional `roster_lookup` for school-tag fallback when partner landed in a different heat), `services/preflight.py` (+150, partner-pool check extended to college, new unresolved-partner check, new non-reciprocal check, new self-reference check, all reuse `name_match` so preflight matches heat-gen exactly), `services/video_judge_export.py` (+15, threads roster_lookup through), `routes/scheduling/heats.py` (+20, single-event flash with held-back names), `routes/scheduling/__init__.py` (+30, bulk-generate aggregates per-event held-back into one Markup flash with Preflight link), `routes/scheduling/heat_sheets.py` (+20, builds roster_lookup for partnered events at all 3 `pair_competitors_for_heat` call sites + new `_build_event_roster_lookup` helper), `templates/scheduling/preflight.html` (+60, expandable detail tables for `unresolved_partner_name` and `non_reciprocal_partnership` issue codes), `tests/test_partner_pairing_fixes.py` (new, 17 tests across Levenshtein, skip-unpaired, roster fallback, preflight extensions), `tests/test_heat_generator.py` (rewrote `test_partnered_unmatched_competitor_is_single` → `test_partnered_unmatched_competitor_held_back_by_default` + added `test_partnered_unmatched_legacy_solo_placement_opt_in` for the legacy `skip_unpaired=False` path).

**Behavioural change.** Partnered-event entrants with unresolved partners are now HELD BACK from heat generation by default. Operator sees a flash listing the names + the unmatched partner string, with a link to Preflight. Legacy callers needing solo placement can opt in via `_build_partner_units(..., skip_unpaired=False)`.

**Regression tests.** 252 passed across `test_partner_pairing_fixes`, `test_partner_resolver`, `test_persisted_config_route_reads`, `test_one_click_and_fnf`, `test_preflight`, `test_routes_smoke`, `test_flight_sizing_modes`, `test_flight_builder_async_spillover`, `test_heat_generator`. 345 passed in the broader heat-gen + partner sweep. Pre-existing flake in `test_flight_builder_25_pros::test_no_competitor_in_consecutive_heats` (7.3% adjacent placements vs 5% threshold) confirmed independent of these changes via stash + re-run on bare main.

**Migration.** None.

### 2026-04-23 (V2.14.9)

**What changed for you.** Fix for a race-week-observed UX dead end on College Birling bracket setup. Operators who seeded the first gender's bracket (Men's or Women's) believed the other gender could not be seeded because clicking the sidebar "Birling Brackets" link took them to a combined PDF containing only the one they had already seeded. The sidebar now routes to a new hub page at `/scheduling/<tid>/birling` listing every birling event with a status badge (Not seeded / Seeded / Complete), Manage button, and Print Blank button per event, plus a single Print All action at the top. Also rewrites the in-app bracket visualization (the one judges see on the Birling management page, not the printable) so matches in each round align with their parents via shared column heights + horizontal connector stubs. Previously a flex-column with `justify-content: space-around` but no shared row height, so boxes floated at random positions with no connecting geometry.

**Why it matters.** The old sidebar link was a navigation dead-end for a race-day workflow — a judge setting up both brackets had to know to navigate through the Friday Event Order card on the Events page to reach the second gender's seeding form. The new index page surfaces both events equally, with per-event status visible at a glance so the judge knows which is unseeded before they click. Bracket visual fidelity matters because Friday evening judges watch the screen during the bracket run — if the layout is incoherent, they cannot follow advancement.

**Files.** `routes/scheduling/birling.py` (+60, new `birling_index` handler), `templates/scheduling/birling_index.html` (new, 85 lines), `templates/_sidebar.html` (3 lines — link endpoint + active-state + removed `target="_blank"`), `templates/scheduling/birling_manage.html` (+35/-16, CSS rewrite + per-row `--row-match-count` CSS variable + `.losers-row` class for irregular loser-bracket geometry), `tests/test_birling_index_route.py` (new, 6 tests).

**Regression tests.** `TestBirlingIndex` + `TestSidebarLink`: zero-state renders, both genders appear with per-event manage URLs, seeded status surfaced in the rendered HTML, Print All reachable, 404 on missing tournament, sidebar now links to `birling_index` (not `birling_print_all`). `300 passed, 6 skipped` across birling + route smoke suites. Two pre-existing failures in `test_flight_builder_25_pros.py` and `test_schedule_generation.py` are from a parallel-session `build_pro_flights(..., num_flights=, commit=False)` signature addition in the unshipped worktree; those tests still mock the old 1-arg signature. Not touched per `feedback_never_stash_unknown_changes.md`.

**No migration.**

### 2026-04-23 (V2.14.8)

**What changed for you.** One-sprint cleanup landing 18 audit findings from a full `docs/solutions/` lessons-learned sweep against the live codebase. Zero race-day breakage fixes (those shipped V2.14.0–V2.14.7 individually). This ship consolidates the long-tail hygiene + perf + doc-drift items the audit surfaced, plus one deferred race-week item (cache invalidation on the pro ability rankings page) held until after V2.14.7 merged. Spectator portal + kiosk + public API now invalidate cached payloads on every mutation of Birling, Event Setup, Friday Showcase config, Ability Rankings, Saturday Priority, event reorder, and flight build — previously those pages served stale scoreboards until the cache TTL expired. Judge sheets stay consistent with validation after a scratch — the `HeatAssignment` rows stay aligned with the authoritative `Heat.competitors` JSON. CSV bulk-import of pro/college results no longer silently drops competitors whose `events_entered` stores event names (Excel / Google Forms imports per CLAUDE.md §4). Operator-facing flash messages no longer leak raw exception text from `except Exception` paths; full traceback goes to app logs, UI surfaces a generic message. Concurrent-edit collisions in relay / axe / birling routes now flash "another judge updated this — reload" instead of an opaque 500. Seven dashboards under "Pro Saturday" and "Virtual Woodboss" have new smoke tests that seed realistic data and assert content, catching the class of V2.14.5 bugs that empty-DB smokes miss. Visual polish: emojis removed from 5 templates per the CLAUDE.md §6 plain-text rule, replaced with Bootstrap Icons.

**Why it matters.** The audit subagents found 87 raw findings across 6 audit tiers, which we triaged to 17 actionable clusters + 2 deferred (sibling-owned file). Of those, 4 were race-week-critical and 5 were should-fix-soon; everything else was tech-debt. Shipping them as one sprint (rather than 17 separate PRs) respects the race-week change budget while still clearing the backlog. Parallel-session coordination: sibling session ran concurrently on `fix/ability-rankings-event-signup-filter` (V2.14.7). Per the sequential-ship pattern documented in `docs/solutions/best-practices/sequential-ship-pattern-parallel-claude-sessions-2026-04-23.md`, this session worked in a `git worktree add` adjacent directory, never touched `routes/scheduling/ability_rankings.py` or its template/test, waited for V2.14.7 to merge, then rebased cleanly and added the deferred ability-rankings cache-invalidation pair. No version-slot collision, no three-way merge, no force-push.

**Three discrete commit phases on the branch** (squashed into a single merge commit on main).

Race-week-critical (commit phase 1):
  1. **Cache invalidation gaps.** `invalidate_tournament_caches(tournament_id)` now called after every commit site in `routes/scheduling/birling.py` (6 mutating routes), `routes/scheduling/events.py` (7 commit sites across `generate_all` / `rebuild_flights` / `integrate_spillover` / `setup_events` / 3 event-reorder routes / `apply_saturday_priority`), `routes/scheduling/friday_feature.py` (4 sites), and — added post-rebase — `routes/scheduling/ability_rankings.py` (2 sites).
  2. **Heat sync after scratch.** `services/scratch_cascade.py::execute_cascade` and both `_remove_*_competitor_from_unfinished_heats` helpers in `routes/registration.py` now call `heat.sync_assignments(heat_type)` after `heat.remove_competitor()`, so `HeatAssignment` rows stop drifting from the `Heat.competitors` JSON.
  3. **CSV import name resolution.** `services/scoring_engine.py::import_results_from_csv` now builds both an int-id set and a normalized-name set from `events_entered`, accepts a row if either matches. Previously name-keyed competitors silently failed — same failure shape as the V2.8.2 Woodboss zero-count bug.
  4. **`flash(str(e))` cleanup.** 5 `except Exception` sites in `routes/reporting.py` (ALA PDF × 2), `routes/registration.py` (gear sync), `routes/import_routes.py` (pro Excel parse), `routes/scheduling/flights.py` + `heats.py` (bare generic catches) now log full traceback via `current_app.logger.exception` and surface a generic "see application logs" message instead of raw exception text.

Should-fix-soon (commit phase 2):
  5. **Empty-DB smoke gaps.** New `tests/test_non_empty_dashboard_smokes.py` seeds realistic data (including round-tripping Partnered Axe state through the real `PartneredAxeThrow.register_pair` emitter rather than hand-writing the fixture per the V2.14.0 anti-pattern) and asserts status 200 + specific content substring for Pro-Am Relay dashboard, Partnered Axe dashboard, Fee Tracker, Pro Payout Summary, Flight List, Build Flights, and Woodboss report — 7 dashboards previously smoked against empty DB.
  6. **JSON.loads guard.** `services/partnered_axe.py::_load_state` bare `except:` replaced with `except (json.JSONDecodeError, TypeError)`. The other sites the audit flagged (`routes/main.py`, `routes/proam_relay.py`, `routes/scheduling/ability_rankings.py`) were already guarded — audit was over-broad.
  7. **List-only event name check + woodboss obstacle pole gender.** `services/schedule_builder.py::_college_friday_sort_key` now uses name-based `LIST_ONLY_EVENT_NAMES` with `is_open` fallback; fixes the "operator toggled Axe Throw to CLOSED and it dropped out of top-of-day" bug class, same root as the V2.14.2 schedule_status fix. `services/woodboss.py::calculate_saw_wood` splits `is_gendered` into `college_gendered` + `pro_gendered` sets so Pro Obstacle Pole (no gender) stops splitting into phantom M+F rows.
  8. **Prod-DB test fixtures.** `tests/test_edge_cases.py` and `tests/test_integration_qa.py` fixtures that copied `instance/proam.db` now SKIP cleanly when the file is absent (CI default). Module docstrings flag the deprecation and point new tests at `tests/fixtures/synthetic_data.py` + `create_test_app()`.
  9. **Cross-event physical-stand collision matrix.** `_CONFLICTING_STANDS` in `services/flight_builder.py` type changed from `dict[str, str]` to `dict[str, set[str]]`; added Stock Saw ↔ Hot Saw (shared stands 1-2) and Obstacle Pole ↔ Speed Climb (shared Pole 2) to the existing Cookie Stack ↔ Standing Block pair. Consumer updated to iterate the set.

Tech-debt (commit phase 3):
  10. **N+1 query fixes.** `services/preflight.py` heat-sync-mismatch loop replaced with 2 batch queries + in-memory join (longest user-visible race-day request); `services/flight_builder.py::build_flight_audit_report` per-flight Heat query replaced with one `in_()` batch + in-memory sort; `services/proam_relay.py::set_teams_manually` per-id competitor queries replaced with two `in_()` bulk queries; `routes/scheduling/flights.py` per-event count + per-heat Event lookup batched.
  11. **Migration PG safety.** `migrations/versions/d8d4aa7bdb45`'s downgrade path flipped Boolean `existing_server_default` from `sa.text('0')` / `sa.text("'0'")` to `sa.text('false')` — rare but real PG rollback fragility.
  12. **`StaleDataError` split.** Routes in `routes/proam_relay.py` (5 handlers), `routes/partnered_axe.py` (4 handlers), `routes/scheduling/birling.py` (3 handlers) now catch `StaleDataError` before `ValueError`, roll back, and flash "Another judge updated this in parallel — reload" per the scoring.py precedent in CLAUDE.md.
  13. **STRATHMARK ephemeral-instance comment.** `services/strathmark_sync.py` now documents the Railway-wipes-`instance/`-on-redeploy limitation and points to the follow-up ticket for moving state to the DB.
  14. **Mock fidelity comments.** `tests/test_schedule_generation.py` lambda mocks at the two call sites for `integrate_college_spillover_into_flights` + `build_pro_flights` now carry MOCK FIDELITY blocks documenting the production signature and flagging drift risk. Full `create_autospec` migration deferred.
  15. **Emoji cleanup.** `templates/portal/kiosk.html`, `templates/scoring/event_results.html`, `templates/scoring/enter_heat.html`, `templates/portal/user_guide.html`, `templates/woodboss/report_print.html` — 13 emoji glyphs (🥇🥈🥉⚠✓✕) replaced with Bootstrap Icons or prose per CLAUDE.md §6 plain-text rule.
  16. **`app.py` context processor → service.** New `services/sidebar_aggregator.py::unscored_heats_count(tid)` now owns the DB query previously inlined in `inject_strings` per CLAUDE.md §6 "no DB logic in app.py" rule.
  17. **CLAUDE.md doc drift.** §2 routes/scheduling/ structure now lists the 4 missing sub-modules (birling, partners, print_hub, pro_checkout_roster) + routes/demo_data.py. §5 auth paragraph updated to list all 12 management blueprints and acknowledge woodboss_public_bp as a third public route family.

**Audit coverage vs reality.** 17 of 17 non-sibling audit categories shipped. Zero items dropped on the floor. Two ability-rankings items surfaced by the audit were folded into this ship after V2.14.7 merged — cache invalidation at both commit sites was still missing; the 3 JSON.loads sites the audit flagged had already been guarded by the sibling's V2.14.7 changes (not our audit, but the same underlying `except (json.JSONDecodeError, TypeError)` pattern).

**Tests.** 220 passed across `test_schedule_status`, `test_one_click_and_fnf`, `test_flight_builder`, `test_flight_builder_async_spillover`, `test_payout_template_render`, `test_non_empty_dashboard_smokes` (new), `test_routes_smoke`, `test_proam_relay_redraw_route`, `test_proam_relay_placement`, `test_partnered_axe_state`, `test_birling_bracket`, `test_pg_migration_safety`, `test_schedule_generation`. Zero regressions. The `test_non_empty_dashboard_smokes` file itself caught a V2.14.0-class fixture-divergence bug during authoring — hand-written axe pair shape didn't match what `PartneredAxeThrow.register_pair` emits; round-tripped through the real service per the trilogy doc rule.

**Files touched.** ~35 files, +896/-116 across three commit phases + V2.14.8 release commit. See branch `fix/sprint-audit-cleanup` for the atomic per-phase commits.

**Meta.** Audit methodology itself is worth preserving. `docs/solutions/` is now load-bearing enough that running 6 parallel audit subagents against the live codebase produced 87 raw findings in ~30 minutes, with high signal-to-noise after triage. Future `/ce-verify`-style sweeps should lean on it.

---

### 2026-04-23 (V2.14.7)

**What changed for you.** The Pro Ability Rankings page at `/scheduling/<tid>/pro/ability-rankings` now shows only the pros who actually signed up for each event, scoped by the event's gender. Previously every active pro appeared in every card regardless of event entry — a judge opening Obstacle Pole saw all 33 men and all 16 women stuck in an unlabeled "Unranked" zone, with no path to rank the people who actually threw the pole. The "Ranked" drop zone is now visibly labeled with a dashed drop target and empty-state text so judges can tell where to drag competitors. Women's Underhand excludes men, Men's Obstacle Pole excludes women, pros who signed up for nothing never appear, and Jack & Jill stays correctly mixed.

**Why it broke.** `routes/scheduling/ability_rankings.py` was loading `ProCompetitor.query.filter_by(tournament_id=..., status='active')` for every category and dropping every pro into every M/F bucket with no `events_entered` check. The template rendered the Ranked zone as an unlabeled 48px-tall empty `<ul>` between the section header and the dashed divider, so the drop target was literally invisible. The fix runs each category's events through `_competitor_entered_event()` (the existing canonical helper from `services/heat_generator.py` / `routes/scheduling/__init__.py`), enforces `event.gender` against `competitor.gender` per event, and dedupes when multiple events feed one category (e.g. Standing Block Speed + Hard Hit both map to `standing_block`). Template now wraps the `<ul>` in a `.ranked-zone` div with a "RANKED" label and a `:empty::before` placeholder ("Drag competitors here to rank them").

**Files touched.** `routes/scheduling/ability_rankings.py` (+49/-27 — `_competitor_entered_event` imported from `.`, GET handler rewritten to filter per category), `templates/scheduling/ability_rankings.html` (+44/-18 — `.ranked-zone` wrapper + `.ranked-label` + empty-state CSS), `tests/test_ability_rankings_event_filter.py` (+177, new — 3 regression tests using module-scoped app + login-based `auth_client` per the `test_proam_relay_redraw_route.py` pattern to sidestep committing-route collisions with the shared conftest `admin_user` fixture), `pyproject.toml` (2.14.6 → 2.14.7), `routes/main.py` (two `/health` + `/health/diag` version literals bumped per PREPARE FOR COMMIT step 5 grep). No migration. Full suite 3298 passed. No `get_last_lh_flight_warnings` / scoring / payouts / migration touched — single-file filter fix plus template label.

**Race-weekend context.** Shipped Thursday night for Saturday show. User reported the bug on the live Railway prod page with a PDF screenshot showing every event category stuck in a list of 33 identical men + 16 identical women with no visible Ranked zone — judges couldn't rank anyone. The fix is a data-layer change plus a two-property CSS delta; no UI behavior depends on JavaScript changes (SortableJS group names unchanged, submit-handler unchanged). Railway auto-deploys from main; expect ~3 min from merge to `/health` returning 2.14.7.
