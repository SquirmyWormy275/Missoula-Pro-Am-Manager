...
def preflight_check(tournament_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'autofix':
            ...
            # D12-C commit F2: the heat-assignment repair clause is gone from
            # this message because the sweep behind it is gone. There is one
            # roster store now, so there is no second copy for it to have
            # drifted from and no repair to report.
            flash(
                f"Auto-fix complete:{gear_msg}{pairs_msg} "
