...
def add_to_heat(tournament_id, event_id):
    ...
    # events_entered stores event NAMES as strings in real registration data,
    # not ids, so int(raw) discarded every value and the gate refused every
    # college late-add. _competitor_entered_event is the canonical resolver
    # already used by heat generation: it matches on id first, then on the
    # normalized name with the pro alias table. Imported under its private
    # name rather than aliased, so there is one definition of "entered".
    from services.heat_generator import _competitor_entered_event
