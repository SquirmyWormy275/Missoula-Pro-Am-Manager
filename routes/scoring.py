...
def _load_competitor_for_tournament(tournament_id: int, competitor_id: int):
    """Return (competitor, tournament) or abort 400/404/403.

    CollegeCompetitor and ProCompetitor are separate tables with independent
    autoincrement primary keys, so the same integer id can name a real person
    in each table.  On 2026 production data ids 29 through 49 collide.  The
    old body tried ProCompetitor first and fell back to CollegeCompetitor,
    which silently resolved every colliding college id to the pro twin.

    Callers pass competitor_type ('college' or 'pro') so the id is read
    against the right table.  request.values covers both the query string
    (GET preview) and the form body (POST confirm/undo).

    Without a type we can only disambiguate when exactly one table has a row
    in this tournament.  If both do, the id is genuinely ambiguous and we
    abort 400 rather than guess; both real entry points send the type, so
    ...
    if comp_type == 'college':
        ...
        if len(in_tournament) > 1:
            abort(400, description=(
                'Ambiguous competitor id: it exists in both the college and '
                'pro rosters for this tournament. Retry with competitor_type.'
            ))
        # Exactly one match, or none — fall through to the guards below,
        # preferring whichever row is actually in this tournament.
        comp = in_tournament[0] if in_tournament else (pro or college)
...
def _competitor_type_of(comp) -> str:
    """Return 'college' or 'pro' for a competitor instance."""
