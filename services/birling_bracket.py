...
class BirlingBracket:
    ...
    def _expected_round_1_match_count(self, n: int) -> int:
        ...
        Mirrors the generate_bracket() logic:
          odd N  → 1 bye (for top seed) + (N - 1) // 2 pairs
          even N → N // 2 pairs
        Used by is_stale_power_of_two_shape() to decide if the stored
        bracket was produced by the pre-V2.14.14 power-of-two generator.
        """
        if n < 2:
            return 0
        if n % 2 == 1:
    ...
    def is_stale_power_of_two_shape(self) -> bool:
        """Detect a bracket generated by the pre-V2.14.14 power-of-two code.

        Pre-V2.14.14: a 9-entrant bracket was upsized to 16 slots, leaving
        8 round-1 matches with the last one silently stacking seed 8 + 9
        into the same competitor1/competitor2 pair instead of a bye.

        Returns True when the stored round-1 match count does not match
        the compact shape the current generator would produce for the same
        entrant count. No-op on freshly-generated brackets.
        """
        winners = (self.bracket_data.get('bracket') or {}).get('winners') or []
        if not winners:
            return False
        stored_r1 = len(winners[0])
        n = len(self.bracket_data.get('competitors') or [])
        expected = self._expected_round_1_match_count(n)
        return expected > 0 and stored_r1 != expected

    def has_any_results_recorded(self) -> bool:
        """True if any match across the whole bracket carries judge-entered state.

        Used as a safety gate by the stale-shape auto-migration: if the
        operator has already entered results, refuse to silently rebuild
        the bracket underneath them.

        "Results" includes BOTH decided matches (winner recorded) AND
        in-progress matches (any fall recorded but no winner yet). Birling
        is best-of-3, so a single fall against seed 2 in a stale W1_2 match
        is real judge work that the rebuild would silently destroy. Only
        bye auto-advances are ignored — those are generated state, not
        operator state.
        """
        bracket = self.bracket_data.get('bracket') or {}
        for round_matches in (bracket.get('winners') or []):
            for m in round_matches:
                if m.get('is_bye'):
                    continue
                if m.get('winner') is not None or m.get('falls'):
                    return True
        for round_matches in (bracket.get('losers') or []):
            for m in round_matches:
                if m.get('winner') is not None or m.get('falls'):
                    return True
        for key in ('finals', 'true_finals'):
            m = bracket.get(key) or {}
            if m.get('winner') is not None or m.get('falls'):
                return True
        if self.bracket_data.get('placements'):
            return True
        return False

    def rebuild_if_stale_shape(self) -> bool:
        """Auto-migrate a stale power-of-two bracket to the compact shape.

        Only runs when both conditions hold:
          - round-1 match count doesn't match compact shape
          - no results have been recorded yet
        Preserves the existing seeding order so judges re-print the same
        matchups they were expecting.  Returns True if a rebuild happened.
        """
        if not self.is_stale_power_of_two_shape():
            return False
        if self.has_any_results_recorded():
            return False
        competitors = self.bracket_data.get('competitors') or []
        seeding = self.bracket_data.get('seeding') or []
        if not competitors or len(competitors) < 2:
            return False
        self.generate_bracket(competitors, seeding=seeding)
        return True
    ...
    def generate_bracket(self, competitors: list, seeding: list = None):
        ...
        # Each round pairs the previous round's winners two-to-one, so the next
        # round holds ceil(prev / 2) matches.  A round holding a single match is
        # the last one: its winner goes straight to the grand finals, so nothing
        # further is appended.  Terminating on prev_count instead of on the
        # newly computed count is what stops ceil(1 / 2) == 1 from looping
        # forever, and is also what stops a 2-competitor field (one first-round
        # match) from growing a phantom W2 round that nothing can ever feed.
        # MAX_WINNERS_ROUNDS is a structural backstop only; prev_count reaching
        # 1 is what ends the loop for any real field.
        MAX_WINNERS_ROUNDS = 32
        prev_count = len(first_round_matches)
        round_num = 2

        while prev_count > 1 and round_num <= MAX_WINNERS_ROUNDS:
            matches_in_round = math.ceil(prev_count / 2)
            round_matches = []
        ...
        # Push the first-round bye winners into their next match.  This runs
        # here, not in the auto-advance loop above, because _advance_winner
        # needs every later round and the finals structures to already exist.
        # Without it a bye winner sits in a decided W1 match forever and the
        # W2 match they should have filled never becomes playable, because
        # _sweep_winners_byes skips any match that already has a winner.
        for match in self.bracket_data['bracket']['winners'][0]:
    ...
    def record_match_result(self, match_id: str, winner_id: int,
        ...
        # Auto-advance lone competitors whose opponent will never arrive
        # (source match was a bye or dead).  The winners sweep is needed as
        # well as the losers sweep: a round fed by an odd number of winners
        # leaves its last match with one live slot and one that nothing can
        # ever fill, and that only becomes visible as results come in.
        self._propagate_byes()
