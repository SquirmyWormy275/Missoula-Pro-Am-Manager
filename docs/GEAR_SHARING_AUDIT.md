...
9. **College Obstacle Pole gear constraints silently ignored** — `services/gear_sharing.py:140-144`. `is_no_constraint_event` returns True for college obstacle_pole because `GEAR_FAMILIES['op_saw'].pro_only=True`. College competitors who write SHARING for the OP Saw still have entries written into `gear_sharing` JSON, but the gear-completeness check, the printable report, and `competitors_share_gear_for_event` (when it walks the family) treat the event as having no constraints. Domain-doc says college competitors do bring/share their own OP saws. Severity: HIGH. Confidence: 8.
...
17. **`build_parse_review` skips competitors who already have any `gear_sharing` entry** — `services/gear_sharing.py:803,815-818`. `already_structured` is True when ANY entry exists in `gear_sharing`. Partial parses (one event matched, other intended events not) hide the row from re-review. Once an entry is committed for one event, the user cannot re-trigger parsing of the same text to capture additional events. Severity: MEDIUM. Confidence: 8.
...
20. **`_parse_dirty_gear_text` output is never written to `ProCompetitor.gear_sharing` JSON** — `services/registration_import.py:390`. `CompetitorRecord.gear_sharing_records` is populated, surfaces in the import report, then discarded — the `to_entry_dicts()` round-trip drops it (it carries `gear_sharing_details` only). The richer parser is effectively unused for production data. Severity: HIGH. Confidence: 9.
