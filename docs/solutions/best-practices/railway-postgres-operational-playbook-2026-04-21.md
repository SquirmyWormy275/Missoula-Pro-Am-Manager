...
**17. Pin the Python interpreter to the production minor version, then verify with AST.** Railway runs Python 3.10. Local dev runs 3.13. Local app-import smokes (`python -c "from app import create_app; ..."`), local `pytest`, and CI `py_compile` all parse against the locally-installed interpreter — so PEP 701 (3.12+) syntax (notably backslash inside an f-string expression part) sails through every gate and crashes the Railway pre-deploy step (`flask db upgrade`) at app import. Production stays on the previous version with no obvious signal beyond the Railway dashboard going red. Defense in depth: (a) `.python-version` file declaring the prod minor version so pyenv / mise / uv default new dev machines correctly; (b) `target-version = "py310"` in `pyproject.toml` `[tool.ruff]`; (c) pre-push reflex `for f in $(git diff --name-only HEAD~1 -- '*.py'); do python -c "import ast; ast.parse(open('$f').read(), feature_version=(3,10))" || echo FAIL: $f; done`; (d) wire the same AST check into CI so the gate runs every PR. The `feature_version=(3,10)` argument applies 3.10's grammar regardless of the locally-installed interpreter, which is the only check that catches version-restricted-but-locally-parseable syntax. See [`../build-errors/railway-python-310-fstring-backslash-deploy-failure-2026-04-23.md`](../build-errors/railway-python-310-fstring-backslash-deploy-failure-2026-04-23.md) for the V2.14.10 incident that surfaced this — third occurrence of the same Python-version-skew foot-gun on this project.
