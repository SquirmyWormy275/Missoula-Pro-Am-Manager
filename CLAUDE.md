...
docs/DOMAIN_CONTRACT.md # Canonical operating contract for core tournament behavior. This wins
                        # when older docs conflict.

FlightLogic.md          # Detailed flight builder and heat-generation algorithm reference.
                        # Keep it aligned with docs/DOMAIN_CONTRACT.md.
...
**Service classes for complex state:** `ProAmRelay`, `PartneredAxeThrow`, and `BirlingBracket` retain their domain logic in service classes, but do not share one storage rule. A complete Relay draw is authoritative in `relay_states`, `relay_teams`, `relay_team_members`, and `relay_team_events`; `event_state` and `payouts` are fallback representations for legacy or incomplete documents that cannot yet be represented safely. Partnered Axe persists state in `event_state` with a legacy `payouts` fallback. Birling reads its five `birling_*` tables when projected, while continuing to dual-write the JSON document in `payouts` until the controlled era-1 repair removes that fallback.

**JSON fields for lists and dicts:** `events_entered`, `partners`, `entry_fees`, `fees_paid`, `gear_sharing`, `payouts`, and `event_state` are stored as JSON text where the relevant service contract calls for them. This is a deliberate simplicity tradeoff for the remaining document-shaped fields, not a substitute for the authoritative Relay, Birling, or HeatAssignment tables. `heats.competitors` and `heats.stand_assignments` were on this list until D12-C phase 2; revision `t9b3c4d5e6f7` dropped both and the roster is `heat_assignments` rows. See the D12-C rule near the end of this file.

**Flexible competitor references:** `EventResult.competitor_id` does not use a SQLAlchemy foreign key to a competitor table. Instead, `competitor_type` (`'college'` or `'pro'`) tells the code which model to query. This allows one Event/Result system to serve both college and pro without polymorphic inheritance. `heat_assignments` is the exception and the direction of travel: revision `s8a0b2c3d4e5` gave it a `uid` foreign key onto the identity spine alongside the legacy pair.
...
Fields: `id`, `tournament_id`, `name`, `event_type` (college/pro), `gender` (M/F/None), `scoring_type` (time/score/distance/hits/bracket), `scoring_order` (lowest_wins/highest_wins), `is_open` (college OPEN/CLOSED flag), `is_handicap` (Boolean, default False — Championship vs. Handicap format; applies to underhand/standing_block/springboard Speed events only; Hard Hit events always Championship; controlled via `config.HANDICAP_ELIGIBLE_STAND_TYPES`), `is_partnered`, `partner_gender_requirement` (same/mixed/any), `requires_dual_runs`, `requires_triple_runs` (Boolean — event uses 3-run cumulative scoring format), `stand_type`, `max_stands`, `has_prelims`, `payouts` (JSON dict position → amount, with legacy Birling fallback), `event_state` (Partnered Axe state and legacy Relay fallback), `status` (pending/in_progress/completed), `is_finalized` (Boolean — scoring locked; payouts distributed; prevents further edits). Relationships: heats, results.

`payouts` is the payout configuration for ordinary events and Relay. Partnered Axe state belongs in `event_state`. Relay tables are authoritative after a complete projection and clear `event_state`; an incomplete legacy document may still be read from `event_state` or `payouts`. Birling tables are authoritative when projected, but its bracket JSON remains a deliberate `payouts` fallback until the era-1 repair is completed.
...
Fields: `id`, `event_id`, `heat_number`, `run_number` (1 or 2), `status`, `version_id` (Integer — optimistic locking), `locked_by_user_id` (FK→users, nullable — exclusive lock held by a scorer), `locked_at` (DateTime, nullable — timestamp when lock was acquired), `flight_id` (nullable FK to flights), `flight_position` (nullable Integer — 1-based display order within a flight). Methods: `set_roster(competitor_type, comp_ids, stands=None)`, `get_competitors()`, `get_stand_assignments()`, `get_stand_for_competitor(competitor_id)`, `competitor_count`, `is_locked()`, `acquire_lock(user_id)`, `release_lock()`. The roster lives in `heat_assignments`; there are no roster columns on `heats` and no per-competitor mutators. `set_roster` writes the whole roster and returns True only if it changed something.

### HeatAssignment

The table heat composition lives in: `id`, `heat_id`, `uid` (FK → `competitors.uid`, unique within a heat), `competitor_id`, `competitor_type`, `stand_number`. This is the only roster store. Through D12-C phase 1 a `Heat.competitors` JSON column existed alongside it and the two could diverge; phase 2 made the rows the sole writable truth and revision `t9b3c4d5e6f7` dropped the columns, so the divergence is not a gap to watch any more, it is unrepresentable.
...
- FNF per-event stand count override (V2.14.1, `routes/scheduling/friday_feature.py::friday_feature` + `templates/scheduling/friday_feature.html`): each eligible FNF event row exposes a `stands_event_<id>` numeric input on the Friday Showcase page. Blank preserves existing `Event.max_stands`; invalid input flashes a warning without crashing; valid `>=1` ints are written directly to the Event row and committed before `generate_event_heats` runs, so regenerated heats respect the new cap. Lets Pro 1-Board run FNF on a different stand count than Saturday Springboard even though both share `stand_type='springboard'` — the per-stand-type override on `/setup-events` couldn't separate them. Tests in `tests/test_one_click_and_fnf.py::TestFNFPerEventStandOverride`.
- Run Show inline flight-sizing form (V2.14.2, `routes/scheduling/events.py::_resolve_num_flights_from_form` + `templates/scheduling/events.html` flight-sizing block above Generate buttons): operators no longer have to leave Run Show to retune flight count. Three-field form (mode radio for count vs minutes; `num_flights`; `target_minutes_per_flight`; `minutes_per_heat`) writes the same `Tournament.schedule_config` keys as `/flights/build` via shared helpers (`_read_flight_sizing_config`, `_persist_flight_sizing_config` reused via import from `routes/scheduling/flights.py`). Both pages read and write the same persisted state — change flight count on either page and the other pre-fills with the new value. `_build_pro_flights_if_possible` now accepts `num_flights` kwarg threaded through `generate_all` and `rebuild_flights` actions.
- Schedule status panel false-positive suppression (V2.14.2, `services/schedule_status.py`): warning aggregator now uses name-based `LIST_ONLY_EVENT_NAMES` for college signup-only events (Axe Throw, Caber Toss, Peavey Log Roll, Pulp Toss) and a name-based `_STATE_MACHINE_PRO_NAMES = {"partneredaxethrow", "proamrelay"}` set for pro events whose progression is managed outside normal Heat rows. Replaces the old `event.is_open`-flag check that flashed phantom "X events have no heats yet" warnings on every Run Show page load when operators configured those events as CLOSED. Old `_is_open_list_only` helper retained for any external callers; new `_is_signup_only_college` and `_is_state_machine_pro` are the authoritative checks. Regression tests in `tests/test_schedule_status.py::TestBuildScheduleStatus`.
...
- Print Hub (V2.13.0, `GET /scheduling/<tid>/print-hub`, `routes/scheduling/print_hub.py`, `services/print_catalog.py`, `templates/scheduling/print_hub.html`): single page listing every printable document in the app (14 entries — 13 fixed + 1 dynamic per-event row). Each row shows red/green "configured" status and never-printed/fresh/stale staleness based on a sha1 fingerprint stored in `print_trackers(tournament_id, doc_key, entity_id)`. Sidebar link under Run Show section. Every existing print route gets one `@record_print(...)` decorator line — response bytes unchanged. Fingerprint functions must include EVERY column the print template renders; a new column added to a template without updating the fingerprint will silently leave the Hub "fresh" when it shouldn't be. Pattern doc: `docs/solutions/best-practices/print-hub-fingerprint-staleness-2026-04-22.md`.
- Print Hub Email Delivery (V2.13.0, `POST /scheduling/<tid>/print-hub/email`, `services/email_delivery.py`, `templates/scheduling/_email_modal.html`): emails any Hub document to one or more recipients via SMTP. Consolidates the SMTP pipeline previously inlined in `routes/reporting.py::_send_ala_email` — the ALA email path was refactored to go through the new service with zero behavior change. Env vars: `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM`, optional `EMAIL_ALLOWED_DOMAINS` (comma-separated allowlist; unset = permissive). Rate-limited 20/min per user. Async via `services.background_jobs.submit(label, fn, *args)` — signature **first arg is the label string, not the function**. SMTP credentials scrubbed from error strings. `PrintEmailLog` table audits every attempt (queued → sent / failed). `PrintDoc.admin_only=True` gates docs (currently only `ala_report`) behind `current_user.is_admin` at the email POST, mirroring the underlying GET route's admin check — prevents a non-admin judge from exfiltrating admin-only content through the server-render email path.
- Pro Saturday Checkout Roster (V2.13.0, `GET /scheduling/<tid>/pro/checkout-roster/print`, `/pdf`, `routes/scheduling/pro_checkout_roster.py`, `templates/scheduling/pro_checkout_roster_print.html`): printable landscape A4 roster, one row per active pro alphabetical, with Name / Events Entered (comma-separated) / Notes box / Present checkbox columns. Scratched pros excluded. Used for Saturday morning check-in.
...
- Payout state protection: `Event.uses_payouts_for_state` now guards only bracket events while Birling's legacy JSON fallback remains in `payouts`; Relay payout configuration and Partnered Axe payout configuration use their separate state stores.
...
- Async flight build chains spillover atomically (V2.14.0 Phase 1, PR #68, `services/flight_builder.py`, `routes/scheduling/flights.py::_build_flights_async`): `build_pro_flights(commit=True)` and `integrate_college_spillover_into_flights(commit=False)` both accept the commit flag so the async inner wrapper can call both with `commit=False` then `db.session.commit()` once — on exception, rollback drops the whole chain. Mirrors `ProAmRelay._save_relay_data(commit=False)` precedent. Prior bug orphaned Chokerman Run 2 + every selected `saturday_college_event_ids` heat with `flight_id=NULL` on every async build. 7 tests in `tests/test_flight_builder_async_spillover.py` including atomicity rollback + schedule_config-is-source-of-truth.
- DAY_SPLIT Run 2 routing + placement-mode toggle (V2.14.0 Phase 2, PR #69, `services/flight_builder.py`, `templates/scheduling/friday_feature.html`): `integrate_college_spillover_into_flights` now loops mandatory-add over `config.DAY_SPLIT_EVENT_NAMES` (Chokerman + Speed Climb M/F) instead of hard-coding Chokerman. `filter_by(run_number=2)` applies to any day-split event — Speed Climb Run 1 stays on Friday, only Run 2 goes Saturday. Chokerman still processed LAST so it closes the show (FlightLogic.md §4.1). New `schedule_config['saturday_college_placement_mode']` key (`roundrobin` default, `cluster` alt) controls Speed Climb distribution. Service function reads from `schedule_config` when `placement_mode` not passed explicitly → 10+ existing call sites pick up the UI choice automatically. Radio toggle on friday-feature page. 13 tests in `test_day_split_mandatory_routing.py`, `test_speed_climb_greedy_fill.py`, `test_chokerman_placement_preserved.py`.
- Flight sizing: minutes-per-flight OR num_flights (V2.14.0 Phase 3, PR #70, `routes/scheduling/flights.py::build_flights`, `templates/pro/build_flights.html`): operators pick (a) target flight duration (default 60 min, 5.5 min per heat → derive `num_flights`) or (b) fixed flight count 2-10 (pre-phase-3 behaviour). Persists to `schedule_config` keys `flight_sizing_mode` + `target_minutes_per_flight` + `minutes_per_heat` + `num_flights` so the form pre-fills. Clamps to [2, 10] with a flash-info when the duration-mode computation exceeds bounds. Live estimate labels update via vanilla JS (CSP-safe — script nonce stamped by `_inject_csp_nonce`). 16 tests in `test_flight_sizing_modes.py` including the canonical "60 heats × 5.5 min / 60 min = 6 flights" math case + schedule_config round-trip.
- Pro-Am Relay in final flight + printable teams sheet (V2.14.0 Phase 4, PR #71, `services/flight_builder.py::integrate_proam_relay_into_final_flight`, `routes/scheduling/heat_sheets.py::relay_teams_sheet`, `templates/scheduling/relay_teams_sheet_print.html`): Pro-Am Relay has no Heat rows — state lives in `Event.event_state` JSON only. New function synthesises a single pseudo-Heat placed at the end of the final flight. Chained BEFORE spillover in all 5 call sites so Chokerman Run 2 appends AFTER the relay. Idempotent on rebuild: wipes existing relay Heat + HeatAssignment rows first. `ProAmRelay.__init__(tournament)` takes Tournament NOT Event; state loaded via `self.relay_data` during `__init__` — there is no public `get_state()` method. Accepts any post-lottery status (`drawn` / `in_progress` / `completed`) — checking only `== 'drawn'` orphans the relay mid-show (V2.14.0 codex hotfix, PR #73). New `/scheduling/<tid>/relay-teams-sheet` route renders landscape WeasyPrint roster (HTML fallback for Railway without cairo/pango). Flight tile shows amber "PRO-AM RELAY" badge with inline link to the teams sheet. Registered in Print Hub catalog under Run Show. 15 tests in `test_proam_relay_placement.py` covering placement, Chokerman-lands-after-relay ordering, status transitions (drawn → in_progress → completed all place), and real team shape (`pro_members` + `college_members`, NOT a combined `members` list).
- Domain Conflict Review Board (V2.14.16, `routes/domain_conflicts.py`, `services/domain_conflicts.py`, `templates/admin/domain_conflicts.html`, `docs/domain_conflicts.json`): admin page at `/admin/domain-conflicts/` lists every contract conflict between code, docs, and operator decisions. Filterable by status/category/severity, click-actions write decisions back to the Git-tracked JSON registry. Sidebar link under Admin (admin-only via `_require_admin` + `can_manage_users` blueprint permission). Backed by `services/domain_conflicts.py::list_conflicts` / `update_conflict` — atomic file writes via `os.replace` after JSON dump. Authority order: `docs/DOMAIN_CONTRACT.md` is canonical; CLAUDE.md and FlightLogic.md cross-reference it.
- DOMAIN_CONTRACT-driven decisions (V2.14.16 — `docs/DOMAIN_CONTRACT.md`): (1) ALL Stock Saw — pro AND college — runs on physical saw stands 7-8 (drops the older pro/college stand distinction). `STAND_CONFIGS['stock_saw']` now `[7, 8]` with labels `Stand 7`, `Stand 8`; `_stand_numbers_for_event` and `_is_stock_saw` no longer gate on `event.event_type == 'college'`; flight-builder `_CONFLICTING_STANDS` adds `stock_saw ↔ saw_hand` because group [5-8] now overlaps the stock-saw stands. (2) Partner pairing rewrite in `services/partner_matching.py`: uses fuzzy `services.name_match.find_partner_match` ladder (catches Mckinley/Mickinley typos), tracks CLAIMS so a one-sided A→B reference holds BOTH for review instead of throwing B back into the auto-pair pool, returns 3-bucket summary (`confirmed_pairs` / `assigned_pairs` / `one_sided_claims`). Auto-assign route flash surfaces all three buckets separately. (3) `services/preflight.BLOCKING_CODES` constant + `get_blocking_issues()` helper define hard-blocker issue codes (unresolved partner, self-reference, non-reciprocal, heat sync mismatch). `build_preflight_report()` returns `report['blocking']` + `report['has_blockers']`; preflight template shows a red blocker banner; events-page JSON exposes per-issue `blocking: true/false`. (4) Heat.competitors ↔ HeatAssignment sync invariants pinned with route-level tests (`tests/test_heat_sync_invariants.py`) — audited all 10 mutation sites already call `sync_assignments`, so no production fix needed; tests prevent future drift. New regression suites: `tests/test_partner_auto_assign_v2.py` (6 tests), `tests/test_heat_sync_invariants.py` (5 tests), `TestBlockingCodes` class in `test_preflight.py` (3 tests).
- LH Springboard Stand 4 + flight-contention flash (V2.14.0 Phase 5, PR #72, `services/heat_generator.py::generate_event_heats` stand-assignment block, `services/flight_builder.py::get_last_lh_flight_warnings`): Stand 4 is hard-coded convention for the LH-configured springboard dummy. When a heat contains a LH cutter, they get `stand_number=4`; others fill 1-3 in competitor-list order. All-RH heats use default assignment (stand 4 still used). Edge case — >1 LH cutters in one heat: first wins stand 4, rest fall back to list order and a `multiple_lh_same_heat` warning is recorded. `build_pro_flights` populates `_last_lh_flight_warnings[tournament_id]` after the post-slice sanity check; `/flights/build` route flashes "LH SPRINGBOARD CONTENTION: Flight N contains M left-handed cutters" to operators. Flight-builder LH penalty in `_score_ordering` still enforces spread-across-flights. `FlightLogic.md §5.3` rewritten. 12 tests across `test_lh_springboard_stand_4.py` (new) + `test_flight_builder_lh_constraint.py` (kept intact — still asserts the spread property this phase preserves).
...
Relay and Birling storage authority: complete Relay state belongs in `relay_states`, `relay_teams`, `relay_team_members`, and `relay_team_events`; read those tables first and write complete rosters there. A complete Relay projection clears `event_state`; a deliberately incomplete legacy document may remain in `event_state` until it can project safely, while `payouts` is a legacy read fallback only. Projected Birling brackets are read from the five `birling_*` tables. Until the controlled era-1 repair is complete, Birling continues to dual-write its `payouts` JSON fallback and readers fall back only when no valid row projection exists. This rule supersedes older release-history descriptions of Relay or Birling JSON storage.
...
This reverses what this file said through D12-C phase 1. `Heat.competitors` was the authoritative JSON and `heat.sync_assignments(event.event_type)` copied it into the rows. D12-C phase 2 inverted that: commit E moved every writer and reader onto the rows, commit F2 deleted `sync_assignments`, `json_competitors`, `json_stand_assignments`, the preflight drift check, the `/heats/sync-check` and `/heats/sync-fix` routes and the autofix sweep, and F3 deleted `_project_json` and dropped both columns in revision `t9b3c4d5e6f7`. Through F2 the projection still rendered the columns on every roster write, so a stale reader looked correct rather than failing loudly; that is why the rule above is worded as a prohibition. As of F3 a stale reader fails loudly instead, because there is no such column to read.
