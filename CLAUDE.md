...
**JSON fields for lists and dicts:** `events_entered`, `partners`, `entry_fees`, `fees_paid`, `gear_sharing`, and `payouts` are stored as JSON text columns. This is a deliberate simplicity tradeoff — no join tables for these relationships. `heats.competitors` and `heats.stand_assignments` were on this list until D12-C phase 2; revision `t9b3c4d5e6f7` dropped both and the roster is `heat_assignments` rows. See the D12-C rule near the end of this file.
...
- Schedule status panel false-positive suppression (V2.14.2, `services/schedule_status.py`): warning aggregator now uses name-based `LIST_ONLY_EVENT_NAMES` for college signup-only events (Axe Throw, Caber Toss, Peavey Log Roll, Pulp Toss) and a name-based `_STATE_MACHINE_PRO_NAMES = {"partneredaxethrow", "proamrelay"}` set for pro events whose progression lives in `Event.payouts` JSON instead of Heat rows. Replaces the old `event.is_open`-flag check that flashed phantom "X events have no heats yet" warnings on every Run Show page load when operators configured those events as CLOSED. Old `_is_open_list_only` helper retained for any external callers; new `_is_signup_only_college` and `_is_state_machine_pro` are the authoritative checks. Regression tests in `tests/test_schedule_status.py::TestBuildScheduleStatus`.
